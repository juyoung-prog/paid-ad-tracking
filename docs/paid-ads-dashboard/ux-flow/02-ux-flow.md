...
| 종합 성과(Performance) | **이 캠페인의 현재 값**을 우리 기준 범위(`PERFORMANCE_STANDARDS`: 플랫폼·지표별 good/weak 문턱, 2024년 이후 우리 캠페인 사분위에서 출발, 제품 설정으로 조정)에 대고 목표별 규칙(`GOAL_PERFORMANCE_RULES`: primary + important 진단)으로 합친 Good/Fair/Weak — primary 상위 → Good(important 하위면 Fair) · 중간 → Fair · 하위 → Weak. 과거 비교군과 무관하므로 성과만 있으면 항상 나온다. 기준 범위가 없는 목표(전환)만 "—". 가중치 점수 없음, 사람이 고른 판정 우선 |
