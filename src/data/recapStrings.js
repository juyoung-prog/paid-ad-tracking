...
export const RECAP_STRINGS = Object.freeze({
  ...
  'recap.table.verdictHint': { en: 'Badge = budget efficiency: cost per result for this campaign\'s goal (Awareness CPM · Traffic CPC · Engagement cost per engagement) against our own standard (median of our campaigns since 2024) — ≤80% Good, 80–120% Fair, >120% Weak. The line below = the same KPI ranked against comparable past campaigns (same platform and goal, other events); blank when fewer than 3 exist. Hover for the numbers.', ko: '배지 = 예산 효율: 이 캠페인 목표의 결과당 비용(인지 CPM · 트래픽 CPC · 참여 참여당 비용)을 우리 기준값(2024년 이후 우리 캠페인 중앙값)과 견줌 — 80% 이하 Good, 80~120% Fair, 120% 초과 Weak. 아래 줄 = 같은 KPI를 과거 비교 캠페인(같은 플랫폼·목표, 다른 이벤트)과 견준 순위, 3개 미만이면 비움. 숫자는 마우스를 올리면 보입니다.', 'zh-Hant': '徽章 = 預算效率：此廣告目標的成果成本（認知 CPM · 流量 CPC · 互動 每次互動成本）對比我們的基準（2024 年起我們廣告的中位數）——≤80% Good、80–120% Fair、>120% Weak。下方一行 = 同一 KPI 對比可比過往廣告（同平台、同目標、其他活動）的排名，少於 3 支則留空。將滑鼠移上可見數字。' },
  'recap.table.vsPast': { en: 'vs past', ko: '과거 대비', 'zh-Hant': '對比過往' },
  'recap.table.standardHint': { en: 'Budget efficiency: {metric} {value} vs our standard {standard} ({pct}% of standard). Standard = median of our own {platform} {goal} campaigns since 2024.', ko: '예산 효율: {metric} {value}, 우리 기준 {standard} 대비 {pct}%. 기준값 = 2024년 이후 우리 {platform} {goal} 캠페인 중앙값.', 'zh-Hant': '預算效率：{metric} {value}，為我們基準 {standard} 的 {pct}%。基準 = 2024 年起我們 {platform} {goal} 廣告的中位數。' },
  'recap.table.standardProvisional': { en: 'Provisional standard — based on a single past campaign.', ko: '임시 기준 — 과거 캠페인 1건 기준.', 'zh-Hant': '暫定基準——僅基於 1 支過往廣告。' },
  'recap.table.noStandard': { en: 'No efficiency standard for this goal yet.', ko: '이 목표의 효율 기준값이 아직 없습니다.', 'zh-Hant': '此目標尚無效率基準。' },
  'recap.table.suggestedNote': { en: 'Suggested — not yet confirmed by a person.', ko: '제안값 — 아직 사람이 확인하지 않았습니다.', 'zh-Hant': '建議值——尚未經人工確認。' },
  'recap.table.noComparison': { en: 'Not enough comparison data — fewer than 3 comparable campaigns on this platform with this goal.', ko: '비교 데이터 부족 — 같은 플랫폼·같은 목표의 비교 캠페인이 3개 미만입니다.', 'zh-Hant': '比較資料不足——同平台、同目標的可比廣告少於 3 支。' },
  'recap.table.peerHint': { en: 'Comparison group: campaigns from other events on the same platform with the same goal. Same phase is preferred when at least 3 exist.', ko: '비교군: 같은 플랫폼·같은 목표의 다른 이벤트 캠페인. 같은 단계가 3개 이상이면 그쪽을 우선합니다.', 'zh-Hant': '比較組：同平台、同目標的其他活動廣告。同階段達 3 支以上時優先採用。' },
