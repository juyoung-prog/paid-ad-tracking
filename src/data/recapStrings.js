...
export const RECAP_STRINGS = Object.freeze({
  ...
  'recap.table.perfBody2': { en: 'The primary metrics depend on the campaign goal and are judged against the team\'s configured performance targets (or the campaign\'s own target when one is set). Supporting Video, Engagement, and Action metrics are used as diagnostics when relevant.', ko: '대표 지표는 목표에 따라 다르고, 이 캠페인의 현재 값을 팀이 설정한 성과 기준(캠페인에 목표치가 있으면 그것)에 댑니다. Video·Engagement·Action 지표는 관련 있을 때만 진단에 씁니다.', 'zh-Hant': '主要指標依目標而定，並以此廣告的現值對比團隊設定的成效基準（若廣告設有目標則用該目標）。Video、Engagement、Action 指標僅在相關時作為診斷。' },
  ...
  'recap.table.perfRule': { en: 'Rule: this campaign\'s {primary} vs the configured targets → within target Good · between Fair · past the weak line Weak{important}. Current values only — past campaigns are never used here.', ko: '규칙: 이 캠페인의 {primary}을(를) 설정된 기준에 대어 기준 안 Good · 사이 Fair · 약함 선 밖 Weak{important}. 현재 값만 쓰고 과거 캠페인은 쓰지 않습니다.', 'zh-Hant': '規則：此廣告的 {primary} 對比設定基準 → 達標 Good · 之間 Fair · 超過弱線 Weak{important}。僅用現值，絕不使用過往廣告。' },
  ...
  'recap.table.perfNoStandard': { en: 'No performance target set for this goal', ko: '이 목표의 성과 기준이 아직 설정되지 않음', 'zh-Hant': '此目標尚未設定成效基準' },
  'recap.table.perfNotEnoughHint': { en: 'No overall verdict — no performance target is configured for this goal\'s primary metric (e.g. CPA). Current metrics are shown as-is.', ko: '종합 판정 없음 — 이 목표의 대표 지표(예: CPA)에 설정된 성과 기준이 없습니다. 현재 지표는 그대로 보입니다.', 'zh-Hant': '無整體判定——此目標的主要指標（如 CPA）尚未設定成效基準。現值照常顯示。' },
