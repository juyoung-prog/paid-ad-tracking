...
export const RECAP_STRINGS = Object.freeze({
  ...
  'recap.table.verdictHint': { en: 'Efficiency = cost per result for this campaign\'s goal (Awareness CPM · Traffic CPC · Engagement cost per engagement · Conversion CPA), ranked against comparable campaigns. Fewer than 3 comparable campaigns → no rating. Hover a badge to see whether it is a suggestion not yet confirmed by a person.', ko: 'Efficiency = 이 캠페인 목표의 결과당 비용(인지 CPM · 트래픽 CPC · 참여 참여당 비용 · 전환 CPA)을 비교 캠페인과 견준 순위입니다. 비교 캠페인이 3개 미만이면 판정하지 않습니다. 배지에 마우스를 올리면 아직 사람이 확인하지 않은 제안값인지 알 수 있습니다.', 'zh-Hant': 'Efficiency = 此廣告目標的成果成本（認知 CPM · 流量 CPC · 互動 每次互動成本 · 轉換 CPA）與可比廣告比較的排名。可比廣告少於 3 支則不評定。將滑鼠移到徽章上可查看是否為尚未經人工確認的建議值。' },
