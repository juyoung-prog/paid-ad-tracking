...
export const RECAP_STRINGS = Object.freeze({
  ...
  'recap.takeaways.platform': { en: 'For this event, {cheaper} reached its audience at {pct}% lower CPM than {pricier}. CPM is comparable across platforms; hook and hold rates are not.', ko: '이번 이벤트에서는 {cheaper}가 {pricier}보다 {pct}% 낮은 CPM으로 도달했습니다. CPM은 플랫폼 간 비교가 되지만 Hook·Hold는 정의가 달라 비교하지 않습니다.', 'zh-Hant': '本次活動中，{cheaper} 以低於 {pricier} {pct}% 的 CPM 觸及受眾。CPM 可跨平台比較，Hook 與 Hold 則不可。' },
  'recap.takeaways.recommendation': { en: 'Consider allocating a larger share of future launch-period budget toward {platform} {phase}, and reviewing {weakPlatform} {weakPhase} before reusing it — the data shows it underperformed, not why.', ko: '다음에는 출시 기간 예산을 {platform} {phase} 쪽으로 더 배분하고, {weakPlatform} {weakPhase}는 재사용 전에 다시 살펴보는 것을 권합니다 — 데이터는 뒤처졌다는 사실만 보여주고 이유는 말해 주지 않습니다.', 'zh-Hant': '建議下次將更多開幕期預算投向 {platform} {phase}，並在重複使用 {weakPlatform} {weakPhase} 前重新檢視——資料顯示其表現落後，但未說明原因。' },
  ...
  'insight.field.weakness': { en: 'What could improve', ko: '개선할 것', 'zh-Hant': '可改進的' },
  'insight.field.reason': { en: 'Why', ko: '이유', 'zh-Hant': '原因' },
  'insight.field.recommendation': { en: 'Recommendation', ko: '제언', 'zh-Hant': '建議' },
  ...
  'pattern.next.shiftToPhase': { en: 'Given the stronger click efficiency of the {phase} phase, consider allocating a larger share of future event budgets toward phases that demonstrate stronger downstream action.', ko: '{phase} 단계의 클릭 효율이 더 높았으므로, 다음 이벤트 예산은 행동으로 이어진 단계 쪽에 더 배분하는 것을 고려하세요.', 'zh-Hant': '鑑於 {phase} 階段的點擊效率較高，未來活動預算可考慮更多投向能帶來後續行動的階段。' },
  'pattern.next.splitByPlatform': { en: 'For a similar event, consider using {reachPlatform} for reach-focused phases and {clickPlatform} where clicks matter — based on this event\'s CPM and CTR, not on assumptions about creative.', ko: '비슷한 이벤트에서는 도달이 목적인 단계에 {reachPlatform}을, 클릭이 중요한 단계에 {clickPlatform}을 쓰는 것을 고려하세요 — 이번 이벤트의 CPM·CTR에 근거한 것이며 소재에 대한 가정은 없습니다.', 'zh-Hant': '類似活動可考慮在重視觸及的階段使用 {reachPlatform}，在重視點擊的階段使用 {clickPlatform}——依據本次活動的 CPM 與 CTR，而非對素材的假設。' },
  'pattern.next.leanOnPlatform': { en: 'For a similar event, consider weighting budget toward {platform}, which led on both reach cost and click efficiency here. Re-check the split if the next event\'s numbers differ.', ko: '비슷한 이벤트에서는 도달 비용과 클릭 효율 모두 앞선 {platform} 쪽에 예산을 더 두는 것을 고려하세요. 다음 이벤트 숫자가 다르면 배분을 다시 보세요.', 'zh-Hant': '類似活動可考慮將預算更多投向在此次觸及成本與點擊效率皆領先的 {platform}。若下次數字不同，請重新檢視分配。' },
