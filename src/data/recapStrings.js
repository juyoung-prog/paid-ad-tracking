...
export const RECAP_STRINGS = Object.freeze({
  ...
  'cell.next.hookThen': { en: 'Retest the opening; check whether early attention turns into {outcome}.', ko: '오프닝을 다시 시험하고 초반 주목이 {outcome}(으)로 이어지는지 봅니다.', 'zh-Hant': '重新測試開場，觀察前段注意力是否轉為{outcome}。' },
  'cell.next.reachThen': { en: 'Test whether efficient reach can drive more {outcome} at the same cost.', ko: '효율적인 도달이 같은 비용에서 더 많은 {outcome}을(를) 만드는지 시험합니다.', 'zh-Hant': '測試有效率的觸及能否在相同成本下帶來更多{outcome}。' },
  'cell.next.keepTest': { en: 'Keep {keep}; test for {test}.', ko: '{keep}은(는) 유지하고 {test}을(를) 시험합니다.', 'zh-Hant': '保留{keep}，測試{test}。' },
  'cell.next.keepBudget': { en: 'Keep {keep}; plan the next daily budget for the {pct}% overspend.', ko: '{keep}은(는) 유지하고 다음 일예산은 {pct}% 초과를 감안합니다.', 'zh-Hant': '保留{keep}，並依 {pct}% 超支規劃下次每日預算。' },
  'cell.next.repeat': { en: 'Repeat the {goal} format; confirm {metric} holds at similar spend.', ko: '{goal} 형식을 반복하고 비슷한 지출에서 {metric}이(가) 유지되는지 확인합니다.', 'zh-Hant': '重複{goal}形式，確認相近花費下 {metric} 是否維持。' },
  'cell.next.improve': { en: 'Test a change aimed at {test}; beat this {metric}.', ko: '{test}을(를) 노린 변경을 시험하고 이번 {metric}을(를) 넘깁니다.', 'zh-Hant': '測試以{test}為目標的變更，超越此次 {metric}。' },
