 ...
 * @property {'good'|'mid'|'bad'|null} verdict - 사람이 Edit에서 고른 평가. 자동 등급은 없다(2026-09-08 제품 결정: 종합 등급 개념 자체를 뺐다) — 표에도 보이지 않고 시트에만 남는다
 ...
 * @property {{ hasData: boolean, strength: Object|null, weakness: Object|null }} insight - buildCampaignInsight() — 표의 What worked · Could improve 재료
...
export function buildRecapRows(eventName, allCampaigns, allRecords, options = {}) {
  ...
  const rows = eventCampaigns.map((c) => {
    ...
    return {
      ...
      // 캠페인 해석(What worked · Could improve) — 표의 두 열. 재료는 벤치마크 구간뿐, 등급이 아니다. 사람이 쓴 note가 우선
...
/**
 * 캠페인 한 줄의 해석 — { strength, weakness }. 각 항목은
 ...
 * @returns {{ hasData: boolean, strength: Object|null, weakness: Object|null }}
 */
export function buildCampaignInsight(row, options = {}) {
  ...
  return { hasData: true, strength, weakness };
}
