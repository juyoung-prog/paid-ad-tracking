...
표에서는 빈 컬럼 제거 규칙(keep)이 자동으로 걷어낸다. 행을 클릭하면 그 자리에서
CampaignDetailPanel이 열린다(아래 항목 참고) — \`Edit on Dashboard\`만 useNavigate를
쓰므로 스토리에서도 MemoryRouter로 감싼다.
...
돌리면 캠페인 두 건이 한 막대로 합쳐진다. 그래서 막대를 눌러도 캠페인 드로어를
바로 열 수 없다(어느 쪽인지 정할 수 없다). 대신 PhaseDetailPanel이 단계 합계
(기간·일일예산·계획 예산·실지출)를 먼저 보여주고, 그 안 캠페인 목록에서 한 건을
고르면 CampaignDetailPanel로 넘어간다.
 ...
 * - 부스팅 게시물 행도 다른 행과 같은 두 줄인가 — 첫 줄은 캡션(표시 이름),
 *   둘째 줄은 기간·일수뿐. `Instagram post: …` 원본은 hover 툴팁에만 있다
 *   (이름 안의 줄바꿈이 한 줄로 접히는지도 같이 본다)
 * - 플랫폼/예산/지출 열이 `$일일예산/day · $총예산 planned · $X spent` 순인가.
 *   성과가 없는 부스팅 게시물 행은 spent 없이 끝나야 한다
 * - 첫 줄 이름에서 매장 코드(`G10_`)와 기간 접미사(`_0617~0707`)가 벗겨지고
 *   원본 이름은 hover(title)에만 남는가 — 둘째 줄은 기간·일수뿐이어야 한다
 * - PLANNED BUDGET이 `$0`이 아닌가 — 총예산 0 + 일일예산 있는 행(부스팅 게시물,
 *   $10/day × 8일)이 $80으로 복원돼 합계에 들어가야 한다. Budget Breakdown의
 *   Total 컬럼도 '—'가 아니어야 한다
 * - 축이 **월 라벨(Jun 2026…) + 규칙적인 주 눈금** 두 단인가 — 눈금 날짜는
 *   phase 시작일이 아니라 일정 간격(1·2·7·14·28일 중 축 폭에 맞는 값)이고,
 *   타임라인 양 끝은 예외 없이 눈금이다. 마일스톤 점선은 없다(격자 + 막대 끝
 *   날짜가 같은 정보를 더 조용히 준다)
 * - 격자 세로선이 실선이고 아주 옅은가 — 주 눈금은 chart.grid, 월 경계는
 *   chart.gridStrong 한 단 진하게
 * - 마지막 하루짜리 phase(8/31)의 **막대와 날짜 라벨이 축 오른쪽 끝을 넘지 않는가**
 *   — 축이 데이터보다 양쪽으로 조금 넓어(전체 기간의 3%) 끝점 원이 잘리지 않는다
 * - 막대 안에 글자가 없는가 — 숫자는 전부 왼쪽 두 열이 말하고, 막대는 기간만
 *   말한다. 시작·종료일은 막대 **밖 아래**에 붙는다
 * - 막대 한 줄을 클릭하면 PhaseDetailPanel이 열리고, 그 안 캠페인을 고르면
 *   CampaignDetailPanel로 넘어가는가
 */
 ...
 * 기간이 길어 축이 늘어나면 막대가 전부 몇 px짜리 점이 된다. 79행짜리 표에
 * 길이가 구분되지 않는 막대만 늘어선 차트는 정보가 0이라(issue/i-1, i-2 — 당시엔
 * 막대 안 글자와 마일스톤 점선까지 겹쳐 더 심했다) 그리지 않는 것이 정상 동작이다.
 *
 * 확인 포인트:
 ...
 * - Performance 탭에 Daily spend 섹션이 **아예 없는가** — 이 스토리는 Event를
 *   고르지 않은 상태라(Daily spend는 Event를 골랐을 때만 그린다) 제목까지 없다.
 *   Event를 고른 뒤 데이터가 없을 때가 `No daily data ...` 한 줄이다 —
