...
const features = [
  ...
  { name: '캠페인 종료 후 결과 보고 (Recap)', desc: '이벤트 하나를 고르면 보고용 문서가 나온다 — 플랫폼별 캠페인 표(숫자 자동) + 과거 유사 캠페인 대비 벤치마크 + 사람이 쓰는 판정·장점·아쉬운 점·배운 점·다음 제언. Reports(진행 확인)와 별도 메뉴. 1단계 숫자·벤치마크·인쇄, 2단계 코멘트 저장, 3단계 Excel·한국어/번체중문·AI 초안 (2026-09 추가)', priority: '필수' },
];
        ...
        <TableContainer sx={{ mb: 4 }}>
          <Table size="small">
            <TableBody>
              ...
              <TableRow>
                <TableCell sx={{ fontWeight: 600, verticalAlign: 'top' }}>보고서 따로 제작</TableCell>
                <TableCell>이벤트(예: G10 Grand Opening)가 끝나면 상사 보고용 결과 보고서를 대시보드 밖의 정적 페이지(beauty-master-dashboard)에 숫자를 손으로 옮겨 만들어 왔음 — 숫자 옮기기가 반복되고, "잘 된 건가"를 판단할 과거 비교 기준이 없음 (2026-09 추가)</TableCell>
              </TableRow>
            </TableBody>
          </Table>
        </TableContainer>
        ...
        <TableContainer sx={{ mb: 4 }}>
          <Table size="small">
            <TableBody>
              ...
              <TableRow>
                <TableCell>이벤트 종료 후 보고서(Recap)가 대시보드 안에서 나옴 → 숫자는 동기화 데이터로 자동, 사람은 판단과 문장만 씀. 과거 유사 캠페인과의 비교가 판단 근거로 따라옴 (2026-09 추가)</TableCell>
              </TableRow>
            </TableBody>
          </Table>
        </TableContainer>
        ...
        <TableContainer sx={{ mb: 4 }}>
          <Table size="small">
            <TableBody>
              ...
              <TableRow>
                <TableCell sx={{ fontWeight: 600 }}>읽기 전용 수신자</TableCell>
                <TableCell>보고를 받는 상사/경영진 — Recap 페이지를 링크로 열어 읽기만 한다(로그인 없음). 언어는 영어가 기본이고 한국어·번체중문으로 확장 예정 (2026-09 추가)</TableCell>
              </TableRow>
            </TableBody>
          </Table>
        </TableContainer>
            ...
            <TableBody>
              <TableRow>
                ...
                <TableCell>
                  ...
                  <Link href="https://github.com/juyoung-prog/Influencer-Tracking" target="_blank" rel="noopener">Influencer Tracking Dashboard</Link>에서 확립된 커스텀 테마 토큰과 운영 툴 컴포넌트 패턴 재사용/참고 · 이벤트 종료 후 결과 보고 페이지(Recap): 숫자 자동 + 유사 캠페인 벤치마크 + 코멘트 저장 + 인쇄/PDF·Excel (2026-09 추가, 3단계)
                </TableCell>
              </TableRow>
              <TableRow>
                <TableCell sx={{ fontWeight: 600, verticalAlign: 'top' }}>제외 (1차 범위)</TableCell>
                <TableCell>트렐로 연동 (대시보드를 광고 기록의 원툴로 사용) · Meta/TikTok Ads API 실시간 자동 수집 (추후 확장 검토) · Google Sheets 각 시트와의 완전 자동 동기화 · 인플루언서 캠페인 퍼널(이전 보고서의 Influencer 탭 — 광고 데이터가 아님) · 인스타그램 계정 전체(오가닉) 지표 자동 수집(광고 API에 없는 값 — Recap 3단계에서 선택 입력만)</TableCell>
              </TableRow>
              <TableRow>
                <TableCell sx={{ fontWeight: 600, verticalAlign: 'top' }}>제약사항</TableCell>
                <TableCell>
                  매장 마스터는 코드 체계(G01, BF1 등)를 유지하며 확장 가능해야 함 · 캠페인은 단일/복수/전체 매장 타겟 세 케이스를 모두 지원 · 메타(조지아/플로리다 계정 분리)·틱톡(통합 1계정) 비대칭 구조를 데이터 모델에 반영 · 1차 범위는 단일 사용자 기준, 향후 다중 사용자(권한/역할) 확장 고려 · 비주얼/컴포넌트는 Influencer Tracking Dashboard와 통일감 유지 · 빌드 여부는 노코드 대안과 비교 검토 후 커스텀 대시보드로 확정 · 매장 귀속(attribution): multi_store/all_stores 캠페인은 매장별 예산을 분배하지 않고 캠페인 단위로만 집계 · Recap 다국어(2026-09): 화면 문자열은 언어별 문자열 표, 사람이 쓰는 문장은 처음부터 en/ko/zh-Hant 칸으로 저장, Recap에만 적용 · Recap 벤치마크: 같은 플랫폼 + 같은 단계끼리, 비율 지표만, 중앙값 + 백분위, 비교군 3개 미만이면 "not enough data" · Recap 접근: 읽기는 로그인 없이 공개, 쓰기만 로그인
                </TableCell>
              </TableRow>
            </TableBody>
          </Table>
        ...
        <TableContainer sx={{ mb: 4 }}>
          <Table size="small">
            <TableBody>
              ...
              <TableRow><TableCell>이벤트가 끝난 뒤 보고서를 대시보드 밖에서 다시 만들지 않는다 — 숫자는 자동으로 채워지고 사람은 판단과 문장만 쓴다 (2026-09 추가)</TableCell></TableRow>
              <TableRow><TableCell>보고서의 모든 비율 지표 옆에 "과거 유사 캠페인 대비 어디쯤인가"가 보인다 — 숫자를 모르는 읽는 사람도 잘 됐는지 알 수 있다 (2026-09 추가)</TableCell></TableRow>
            </TableBody>
          </Table>
        </TableContainer>
