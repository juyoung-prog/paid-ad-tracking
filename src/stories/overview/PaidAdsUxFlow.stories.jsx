...
const scenarios = [
  ...
  },
  {
    ...
    flow: [
      ...
      '이벤트 클릭 → /recap/{event}: 머리글(이벤트·기간·매장·플랫폼·계획 예산 대비 지출), 단계 타임라인(행 클릭 → 표의 그 단계 줄 펼침), 플랫폼별 캠페인 표(오른쪽 끝 두 열 What worked · Could improve; 줄 클릭 → 캠페인 상세 드로어: 성과·페이싱·일별 지출)',
...
├── /stores — 매장 마스터 관리
│   └── 매장 리스트 + 추가/수정 폼
├── /reports — Performance (레일 이름, 2026-09 개명) — 진행 중 성과 분석
│   ├── 기간 · 매장 · 플랫폼 선택
│   ├── 요약 통계
│   └── 내보내기 (CSV/이미지)
├── /settings (신규 — API Integration) — 플랫폼 계정 연결 관리
│   └── 계정별(Meta-GA/Meta-FL/TikTok) 연결 상태 + Connect/재연결 CTA
└── /recap — Reports (레일 이름, 2026-09 개명) — 캠페인 종료 후 결과 보고
    ├── 이벤트 목록 (최근 종료 순, Draft/Ready 상태)
    └── /recap/:event — 이벤트 하나의 보고서
        ...
        ├── (편집 모드) 표 안에서 What worked · Could improve 편집 — 예전 판정 · 장점 · 아쉬운 점 · 이유 (언어별) · Learnings 편집기 · ⋯ 수기 입력 팝오버(2026-09-14 제거)
...
const pageList = [
  ...
  { page: 'Performance (레일 이름 — 내부 /reports)', path: '/reports', data: 'Campaign(R), PerformanceRecord(R)' },
  { page: 'Settings (신규)', path: '/settings', data: "Connection(R, connections_public view만 — 토큰 필드는 프론트에 노출 안 함)" },
  { page: 'Reports (레일 이름 — 내부 /recap, 신규 2026-09)', path: '/recap', data: 'Campaign(R, 이벤트 묶음), EventRecap(R, 상태)' },
  { page: 'Report Detail (내부 /recap/{event}, 신규 2026-09)', path: '/recap/{event}', data: 'Campaign(R), PerformanceRecord(R), PerformanceDaily(R), Plan(R), EventRecap(R/W), RecapCampaignNote(R/W) — 벤치마크는 저장 없이 계산' },
];
...
const entities = [
  ...
  },
  {
    ...
    fields: [
      ...
      { field: 'thumbnailUrl', type: 'string | null', format: '이미지(업로드 전용)', desc: '소재 미리보기 이미지. 없으면 CampaignThumbnail이 중립색 이니셜로 대체. creativeUrl과는 별개 값', example: '"data:image/png;base64,..."' },
  ...
  },
  {
    ...
    note: 'Recap 벤치마크는 저장하지 않는 계산 전용: 비교군 = 같은 platform + 같은 goal + 같은 단계의 다른 이벤트 캠페인(3개 미만이면 같은 platform + 같은 goal, 그래도 미만이면 순위 없음) · 대상은 비율 지표(CPM·CTR·CPC·CPE·CPA·Hook·Hold·참여율) · 중앙값 + 백분위 + N · Primary KPI = 이 캠페인의 goal별 KPI 값 자체(자동 등급 없음, 과거 무관), 과거 비교 = 같은 비교군 백분위 순위, 집행률 = 계획 대비 — 세 층을 섞지 않는다 · CPE = 지출 ÷ (좋아요+댓글+공유) · 2024년 이후만 · Meta와 TikTok을 섞지 않는다 · 집행률(pacing)은 성과와 분리.',
  },
];
...
const components = [
  ...
  { name: 'CampaignThumbnail', usage: '캠페인 소재 썸네일', type: '재활용', note: 'components/media/CampaignThumbnail.jsx — thumbnailUrl 없으면 중립색 이니셜 fallback' },
  ...
  { name: 'PerformanceForm', usage: '성과 지표 입력 폼 (goal 기반 Tier 조건부 노출 + Social Metrics 항상)', type: '신규(구현됨)', note: '카테고리: templates' },
  { name: 'SocialMetricsFields', usage: '참여 내역 중 사람이 고칠 수 있는 칸(Likes · Comments · Shares · Saves · Reposts)의 플랫폼별 입력 그리드 — 수기 폼과 동기화 캠페인 드로어 공용, 고친 칸은 "edited"', type: '신규(구현됨, 2026-09-11)', note: '카테고리: templates — 칸 구성은 schema SOCIAL_METRIC_KEYS·editableSocialMetricKeysFor. Follows·Visits는 수정 금지' },
  ...
  { name: 'Print stylesheet (Recap)', usage: '인쇄/PDF — @page Letter 세로. 대시보드 컴포넌트가 그대로 인쇄된다(인쇄 전용 보고서 없음)', type: '신규(구현됨) — Recap 1단계, 세로 단일화 2026-09-10', note: '컴포넌트가 아니라 @page + 각 컴포넌트의 @media print. PaidAdsShell GlobalStyles + data-print 속성' },
];
