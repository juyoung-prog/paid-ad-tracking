...
const recapEnums = [
  ...
  { name: 'METRIC_HEALTH_SCALE / GOAL_PERFORMANCE_RULES / buildPerformanceVerdict(row, platform, goal)', value: "플랫폼·지표별 healthy/poor 두 눈금의 고정 건강 척도 + 목표별 { primary, important, secondary } 지표 표 + 구간 규칙(primary 상위 → Good, important 하위면 Fair · 중간 → Fair · 하위 → Weak · 구간 없으면 null + reason noData/noScale) — 표의 Performance 열. **이 캠페인의 현재 값**만 읽고, 캠페인 목표치(vs target)도 과거 캠페인 분포·중앙값·백분위·순위(vs past)도 관여하지 않는다(2026-09-07). 가중치 점수 없음" },
];
