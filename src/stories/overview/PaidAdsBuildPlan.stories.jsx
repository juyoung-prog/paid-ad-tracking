...
const recapTypes = [
  ...
  { name: 'RecapCampaignRow', kind: '계산 전용', fields: 'getGoalMetricsRow(...)의 모든 필드 + storeCode, phaseName, dailyBudget, rank, benchmarks: Record<metricKey, BenchmarkStat>, goalResult: { goal, primary[], supporting[] }, budgetEfficiency, kpiTarget, note: RecapCampaignNote|null', note: '표 한 행. 정렬·순위까지 끝난 상태로 컴포넌트에 내려간다' },
...
const recapFunctions = [
  ...
  { name: 'buildRecapRows(eventName, campaigns, records, options)', io: '→ { byPlatform: Record<platform, RecapCampaignRow[]>, peerEvents: string[] }', note: '이벤트 캠페인마다 getGoalMetricsRow → benchmarks → goalResult → 플랫폼별로 대표 지표 백분위 순 정렬 후 rank 부여. notesById를 주면 note를 붙인다' },
...
const recapPhases = [
  ...
  },
  {
    ...
    items: [
      ...
      'VerdictChip — data-display. props: verdict(VERDICT|null), lang. good=success / mid=중립 / bad=warning 옅은 틴트 + 옅은 실선 테두리. 자동 등급을 없앤 뒤(2026-09-08) 표·편집기에서는 쓰지 않는다 — 사람이 고른 평가를 보여줄 자리가 생기면 다시 쓴다',
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      '캠페인 해석 → 별도 Notes 카드 대신 캠페인 상세 드로어의 Campaign insights 섹션으로(RecapCampaignInsightPanel layout=grid, 예산 페이싱 뒤). 한때 표의 줄 아래 펼침(셰브론)이었으나 찾기 어려워 뺐다. What worked · Could improve · Why · Next action, 결론 → ↗↘ 근거 한 줄. schema.js buildCampaignInsight()가 벤치마크 구간에서만 재료를 만들고 원인은 지어내지 않는다. 표의 숫자 줄 전체 클릭 = 드로어, 타임라인 행 클릭 = 그 줄로 스크롤 + 선택 표시',
