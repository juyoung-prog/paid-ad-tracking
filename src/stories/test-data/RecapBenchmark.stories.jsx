...
- 종합 성과(performance / suggestedVerdict) = 이 캠페인의 현재 값을 PERFORMANCE_STANDARDS에 댄 구간을 GOAL_PERFORMANCE_RULES로 합침: primary 상위 → Good(important 진단 하위면 Fair) · 중간 → Fair · 하위 → Weak · 기준 범위 없는 목표(전환)는 null. 과거 비교군과 무관. 사람이 고른 note.verdict가 우선
                ...
                <TableBody>
                  {rows.map((r) => (
                    <TableRow key={r.campaignId}>
                      ...
                      <TableCell>{r.suggestedVerdict ?? 'null'}</TableCell>
