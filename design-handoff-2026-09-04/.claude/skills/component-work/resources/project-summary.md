...
스타터킷 위에 **BeautyMaster Influencer Dashboard**가 올라가 있다 — 뷰티 매장 오프닝 캠페인의
인플루언서 방문·업로드·크레딧 이행을 구글 시트에서 읽어 보여주는 운영 도구다.

- 진입: `/beautymaster` (`src/pages/beautymaster/BeautymasterDashboard.jsx`)
- 화면: Operations(일하는 화면) · Report(보고) · Workflow(참조) 세 뷰 + 상세 Drawer
- 데이터: 구글 시트 CSV를 30초마다 폴링. 설정은 localStorage. 백엔드 없음
- 스토리북 문서: `Overview/BeautyMaster/*`, 실사용 토큰은 `Style/*`

디자인 시스템 컴포넌트(`Component/*`, `Interactive/*`)는 카탈로그 목적이라 대시보드에 안 쓰이는 게 정상이다.
